...
Produces:
  ...
  data/synthetic/truth.bed       – implanted crosslink positions (ground truth)
...
def make_truth(path: str) -> None:
    """Write the implanted crosslink positions as BED6.

    Forward clusters truncate at the read start; reverse clusters truncate at
    the rightmost mapped base. Both are recorded as 1 bp features. PureCLIP may
    report a site a base or two away, so scoring uses a window (see compare.py).
    """
    rows = [(pos1 - 1, pos1, "+") for pos1, _ in FWD_CLUSTERS]
    rows += [(x1 - 1, x1, "-") for x1, _ in REV_CLUSTERS]
    with open(path, "w") as fh:
        for start, end, strand in sorted(rows):
            fh.write(f"{CONTIG}\t{start}\t{end}\txlink\t0\t{strand}\n")
...
def main() -> None:
    ...
    truth_path = os.path.join(OUT_DIR, "truth.bed")

    if all(os.path.exists(p) for p in (bam_path, bai_path, fa_path, truth_path)):
        print("Synthetic data already present — skipping generation.")
        return

    os.makedirs(OUT_DIR, exist_ok=True)
    make_truth(truth_path)
    ...
    n_truth = len(FWD_CLUSTERS) + len(REV_CLUSTERS)
    print(f"Done — {total_reads} reads, {CONTIG_LEN} bp reference, "
          f"{n_truth} implanted crosslinks.")
